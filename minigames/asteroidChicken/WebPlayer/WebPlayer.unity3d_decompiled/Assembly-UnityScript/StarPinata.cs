...
[Serializable]
public class StarPinata : MonoBehaviour
{
	...
	public GameObject confettiEmitter1;

	public GameObject confettiEmitter2;
	...
	public override void Setup(float size, float velocityScale)
	{
		...
		num *= velocityScale;
		this.randTorqueX = Random.Range((float)0, 1f);
	...
	public override void Explode()
	{
		...
		RuntimeServices.SetProperty(obj3, "size", 2);
	...
	public override void SpewBeans()
	{
		...
		{
			float num = this.beanWorldSize * 0.5f;
			float num2 = this.get_transform().get_position().x + Mathf.Lerp(-num, num, Random.get_value());
			float num3 = this.get_transform().get_position().y + Mathf.Lerp(-num, num, Random.get_value());
			float num4 = this.get_transform().get_position().z + Mathf.Lerp(-num, num, Random.get_value());
			int num5 = Random.Range(0, 4);
			int num6 = num5;
			if (num6 == 0)
			{
				Object.Instantiate(this.jellybeanPrefab, new Vector3(num2, num3, num4), this.get_transform().get_rotation());
			}
			else if (num6 == 1)
			{
